...
class _HomePageState extends State<HomePage> {
  ...
  void _muatUlang() {
    setState(() {
      _futureCatatan = ApiClient.instance.getAll().then((list) {
            ...
            child: FutureBuilder<List<Catatan>>(
              ...
              builder: (context, snapshot) {
                ...
                if (snapshot.hasError) {
                  ...
                  String pesan = 'Terjadi kesalahan: $e';
                  IconData iconData = Icons.error_outline;
                  Color iconColor = Colors.red;

                  if (e is ApiException) {
                    if (e.statusCode == 404) {
                      pesan = 'Endpoint tidak ditemukan (HTTP 404).\nPastikan Base URL sudah benar.';
                      iconData = Icons.dns_outlined;
                      iconColor = Colors.orange;
                    } else if (e.statusCode == 401) {
                      pesan = 'HTTP 401: API key tidak valid.\nPastikan _apiKey di ApiClient sudah benar.';
                      iconData = Icons.lock_outline;
                      iconColor = Colors.redAccent;
                    } else if (e.statusCode == 0) {
                      pesan = 'Tidak dapat menghubungi server.\nPastikan internet aktif dan domain Base URL valid.';
                      iconData = Icons.wifi_off;
                      iconColor = Colors.grey;
                    } else {
                      pesan = e.message;
                    }
                  } else if (e.toString().contains('SocketException') || e.toString().contains('host lookup')) {
                    pesan = 'Gagal resolve domain server.\nPastikan Base URL yang digunakan valid.';
